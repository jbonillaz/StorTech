<?php 
	...
	class Roles extends Controllers{
		...
		public function getSelectRoles(){
			//Para extraer los nombres de los roles desde los roles.
			$htmlOptions = "";
			$arrData = $this->model->selectRoles();
			if(count($arrData) > 0 ){
				for ($i=0; $i < count($arrData); $i++) { 
		...
		// Para extraer los datos del rol.
		public function getRol(int $idrol){
			...
			if($intIdrol > 0){
				...
				echo json_encode($arrResponse,JSON_UNESCAPED_UNICODE);//conversion a un formato json.
			}
			die();
		}
		...
		public function setRol(){
			...
			if($intIdrol == 0){
				//Crear
				$request_rol = $this->model->insertRol($strRol, $strDescipcion,$intStatus);//Validamos la respuesta del modelo.
				$option = 1;
			}else{
				//Actualizar

		...
		public function setUsuario(){
			if($_POST){
				...
				if(empty($_POST['txtIdentificacion']) || empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono']) || empty($_POST['txtEmail']) || empty($_POST['listRolid']) || empty($_POST['listStatus']) ){
					...
					$idUsuario = intval($_POST['idUsuario']);//intval Convierte a un entero lo que venga en identificacion.
					$strIdentificacion = strClean($_POST['txtIdentificacion']);
					$strNombre = ucwords(strClean($_POST['txtNombre']));//ucwords Comvierte cada letra de la primer palabra en mayuscula.
					$strApellido = ucwords(strClean($_POST['txtApellido']));
					$intTelefono = intval(strClean($_POST['txtTelefono']));
					$strEmail = strtolower(strClean($_POST['txtEmail'])); //strtolower Convierte todas las letras en minisculas.
		...
		public function getUsuarios(){
			...
			for ($i=0; $i < count($arrData); $i++) {
				...
				if($arrData[$i]['status'] == 1){
					...
					// Validaciones para las acciones
